...
if (!empty($TESTID) && !empty($STUDENT)) {
    ...
    $dup = mysqli_query($con,"SELECT * FROM studentTest;");
    $TOTALSCORE = 0;
    while ($check = mysqli_fetch_array($dup)) {
        if ($STUDENT == $check['studentId'] && $TESTID == $check['testId']) {
            $inProgress = 1;
        }
    }
    ...
    if ($inProgress == 0) {
        ...
        echo "<inprogress>".$inProgress."</inprogress>";
        ...
        $print = mysqli_query($con,"SELECT * FROM studentTest;");
        while ($check = mysqli_fetch_array($print)) {
            if ($STUDENT == $check['studentId'] && $TESTID == $check['testId']) {
                echo "<inprogress>".$inProgress."</inprogress>";
                echo "<timestart>".$check['startTime']."</timestart>";
                echo "<timeend>".$check['endTime']."</timeend>";
            }
        }
    }
}
